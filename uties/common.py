...
# (239, 121, 162)
def drawbbox(lable,image, bbox, color=None, thickness=2, textcolor=(0, 0, 0), landmarkcolor=(0, 0, 255)):
    if color is None:
        color = randcolor(bbox.label)

    # text = f"{bbox.label} {bbox.score:.2f}"
    text = lable+f"-{bbox.score:.2f}"
    x1, y1, x2, y2 = intv(bbox.box)
    w = x2 - x1 + 1
    h = y2 - y1 + 1

    cv2.rectangle(image, (x1, y1, x2 - x1 + 1, y2 - y1 + 1), color, thickness, 16)

    border = thickness / 2
    pos = (x1 + 3, y1 - 5)
    cv2.rectangle(image, intv(x1 - border, y1 - 21, w + thickness, 21), color, -1, 16)
    cv2.putText(image, text, pos, 0, 0.5, textcolor, 1, 16)

    if bbox.haslandmark:
        for i in range(len(bbox.landmark)):
            x1, y1 = bbox.landmark[i][:2]
            cv2.circle(image, intv(x1, y1), 3, landmarkcolor, -1, 16)
